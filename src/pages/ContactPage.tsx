...
const ContactPage = () => {
  ...
  useEffect(() => {
    if (activeTab === 'booking' && !isCalendlyLoaded) {
      ...
      const initCalendly = () => {
        if (window.Calendly) {
          window.Calendly.initInlineWidget({
            url: 'https://calendly.com/tejasmalhan2209/30min',
  ...
  const toggleTab = (tab: 'message' | 'booking') => {
    setActiveTab(tab);
  };
      ...
      <main className="pt-32 pb-24 px-4">
        <div className="container mx-auto max-w-6xl">
          <div className="text-center max-w-3xl mx-auto mb-20">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mt-2 mb-6 text-white">Contact Me</h1>
            <p className="text-white/70">
              Have a project in mind? I'm ready to bring your vision to life.
              ...
              <div className="royal-card">
                <div className="flex mb-6 border-b border-white/10">
                  <button 
                    ...
                    className={`px-6 py-4 font-semibold transition-colors ${activeTab === 'message' ? 'text-royal-gold border-b-2 border-royal-gold' : 'text-white/70 hover:text-white'}`}>
                    Send a Message
                  </button>
                  <button 
                    onClick={() => toggleTab('booking')}
                    className={`px-6 py-4 font-semibold transition-colors ${activeTab === 'booking' ? 'text-royal-gold border-b-2 border-royal-gold' : 'text-white/70 hover:text-white'}`}>
                    Book an Appointment
                  </button>
                </div>
                ...
                {activeTab === 'message' ? (
                  ...
                  <div className="aspect-video w-full bg-transparent min-h-[600px]">
                    <div id="calendly-container" style={{ width: '100%', height: '100%', minHeight: '600px' }}></div>
                  </div>
                )}
              </div>
            </div>
...
const SocialButton = ({ href, icon, label }: { href: string; icon: JSX.Element; label: string }) => (
  <a 
    ...
    className="flex flex-col items-center justify-center p-4 bg-royal-gold/10 rounded-lg hover:bg-royal-gold/20 transition-all text-royal-gold"
  >
    {icon}
...
const SuccessMessage = () => (
  <div className="bg-green-500/20 border border-green-500/30 p-6 rounded-lg text-white">
